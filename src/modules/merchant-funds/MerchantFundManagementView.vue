...
import { Eye, LoaderCircle, RefreshCw, RotateCcw, Search, WalletCards } from "lucide-vue-next";
import { ElDatePicker, ElOption, ElPagination, ElSelect, ElTabPane, ElTable, ElTableColumn, ElTabs } from "element-plus";
...
const accountPage = ref({ current: 1, pageSize: 20, total: 0 });
const transactionPage = ref({ current: 1, pageSize: 20, total: 0 });
...
const refresh = () => activeTab.value === "accounts" ? loadAccounts() : loadTransactions();
...
<template>
  <section class="workspace-panel configuration-center management-list-page merchant-fund-management">
    <div class="panel-title"><div><span class="eyebrow">MERCHANT FUNDS</span><h3>商户资金</h3></div><button class="icon-btn" type="button" title="刷新当前列表" :disabled="loadingAccounts || loadingTransactions" @click="refresh"><RefreshCw :class="{ spin: loadingAccounts || loadingTransactions }" :size="16" /></button></div>
    <ElTabs v-model="activeTab" class="merchant-fund-tabs" @tab-change="changeTab">
      ...
      <ElTabPane name="transactions"><template #label><span class="merchant-fund-tab-label"><WalletCards :size="16" />余额明细</span></template>
        ...
        <div class="management-pagination"><ElPagination background layout="total, sizes, prev, pager, next" :current-page="transactionPage.current" :page-size="transactionPage.pageSize" :page-sizes="[20, 50, 100]" :total="transactionPage.total" :hide-on-single-page="false" @current-change="loadTransactions" @size-change="changeTransactionPageSize" /></div>
      </ElTabPane>
    </ElTabs>
  </section>
