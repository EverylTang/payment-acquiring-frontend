...
<template>
  <section class="panel workspace-panel management-list-page">
    <form class="management-filter-form" @submit.prevent="search">
      ...
      <div class="management-filter-actions"><button class="outline-btn" type="button" @click="resetFilters">重置</button><button class="primary-btn" type="submit"><Search :size="16" />查询</button><button v-if="hasPermission('system:user:create')" class="primary-btn" type="button" @click="openCreateDrawer"><Plus :size="16" />新增用户</button></div>
    </form>
  ...
  <AppDrawer v-if="createDrawerOpen" title="新增用户" description="SYSTEM USERS" @close="closeCreateDrawer">
    <form class="drawer-section user-form" @submit.prevent="create">
      ...
      <button v-if="hasPermission('system:user:create')" class="primary-btn drawer-submit" type="submit" :disabled="saving || !form.username || !form.displayName || form.password.length < 12 || !form.roles.length"><Save :size="16" />创建用户</button>
    </form>
  </AppDrawer>
  <AppDrawer v-if="editDrawerOpen && selected" title="编辑用户" :description="selected.username" @close="closeEditDrawer">
    <form class="drawer-section user-form" @submit.prevent="saveUser">
      ...
      <button v-if="hasPermission('system:user:update')" class="primary-btn drawer-submit" type="submit" :disabled="saving || !selected.displayName || !selected.roles.length"><Save :size="16" />保存用户</button>
    </form>
    <section class="drawer-section user-form-section">
      ...
      <button v-if="hasPermission('system:data-scope:user:update')" class="outline-btn drawer-submit" :disabled="saving" @click="saveScope">保存商户范围</button>
    </section>
    <section class="drawer-section user-form-section">
      <h4>重置密码</h4>
      ...
      <button v-if="hasPermission('system:user:password:reset')" class="danger-btn drawer-submit" :disabled="saving || resetPassword.length < 12" @click="savePassword"><KeyRound :size="16" />重置密码</button>
    </section>
  </AppDrawer>
</template>
