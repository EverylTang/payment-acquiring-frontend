...
const hasSelectedRouteProduct = computed(() => activeProducts.value.some((item) => item.productCode === routeForm.value.productCode));
const hasSelectedRouteMerchant = computed(() => activeMerchants.value.some((item) => item.merchantId === routeForm.value.merchantId));
const hasSelectedRouteChannel = computed(() => routeChannels.value.some((item) => item.channelId === routeForm.value.channelId));
const hasSelectedRouteCurrency = computed(() => currencies.value.some((item) => item.code === routeForm.value.currency));
const channelStatusLabel = (status: Channel["status"]) => ({ ACTIVE: "启用", DISABLED: "已禁用", INACTIVE: "未启用" })[status];
...
const load = async () => {
  ...
  try {
    ...
    if (props.section === "routing") {
      ...
      products.value = (values[3] as { items: Product[] }).items;
      merchants.value = (values[4] as { items: Merchant[] }).items;
      routeChannels.value = (values[5] as { items: Channel[] }).items.filter((item) => item.status === "ACTIVE");
      currencies.value = values[6] as Currency[];
          ...
          <table class="data-table configuration-table">
            ...
            <tbody>
              <tr v-for="item in channels" :key="item.channelId">
                ...
                <td class="actions"><button v-if="canEdit('channel')" class="outline-btn action-btn" title="编辑渠道" @click="editChannel(item)"><Pencil :size="15" />编辑</button><button v-if="canToggle('channel')" class="outline-btn action-btn" title="切换渠道状态" @click="toggle('channel', item.channelId, item.status)"><ToggleLeft :size="16" />{{ item.status === 'ACTIVE' ? '停用' : '启用' }}</button></td>
              </tr>
            </tbody>
          </table>
        </div>
