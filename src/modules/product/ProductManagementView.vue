...
const loadCapabilities = async (product: Product) => {
  selected.value = product;
  capabilities.value = (await getProductCapabilities(product.productCode)).items;
};
const save = async () => {
  saving.value = true;
  try {
...
const edit = async (product: Product) => {
  ...
  await loadCapabilities(product);
};
...
const saveCapability = async () => {
  ...
  saving.value = true;
  try {
    const result = editingCapability.value
    ...
    capabilities.value = editingCapability.value
      ? capabilities.value.map((item) => item.capabilityId === result.capabilityId ? result : item)
