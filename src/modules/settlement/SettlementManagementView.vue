...
<template>
  <section class="workspace-panel configuration-center management-list-page settlement-management">
    <div class="panel-title"><div><span class="eyebrow">SETTLEMENT CONTROL</span><h3>结算管理</h3></div><div class="button-row"><button class="primary-btn" type="button" @click="openCreate"><Plus :size="16" />新增规则</button><button class="outline-btn" type="button" @click="openBatch"><Play :size="16" />手动执行</button><button class="icon-btn" type="button" title="刷新" :disabled="loading" @click="load()"><RefreshCw :class="{ spin: loading }" :size="16" /></button></div></div>
