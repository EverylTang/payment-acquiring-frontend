...
<template>
  <section class="workspace-panel configuration-center management-list-page settlement-management">
    <div class="panel-title"><div><span class="eyebrow">SETTLEMENT CONTROL</span><h3>结算管理</h3></div><div class="button-row"><button v-if="hasPermission('settlement:rule:manage')" class="primary-btn" type="button" @click="openCreate"><Plus :size="16" />新增规则</button><button v-if="hasPermission('settlement:batch:run')" class="outline-btn" type="button" @click="openBatch"><Play :size="16" />手动执行</button><button class="outline-btn action-btn" type="button" title="刷新" :disabled="loading" @click="load()"><RefreshCw :class="{ spin: loading }" :size="16" />刷新</button></div></div>
    ...
    <div class="settlement-table-shell"><ElTable v-loading="loading" :data="rules" empty-text="暂无结算规则" border stripe table-layout="fixed" row-key="id"><ElTableColumn label="商户 / 产品" min-width="220"><template #default="scope"><strong>{{ scope.row.merchantId }}</strong><small class="table-subtext mono">{{ scope.row.productCode || "历史通用规则" }}</small></template></ElTableColumn><ElTableColumn label="币种 / 周期" min-width="180"><template #default="scope"><strong>{{ scope.row.currency }}</strong><small class="table-subtext">{{ cycleSummary(scope.row as SettlementRule) }}</small></template></ElTableColumn><ElTableColumn label="结算门槛" min-width="145"><template #default="scope"><span class="settlement-amount">{{ scope.row.minSettlementAmount }}</span><small class="table-subtext">费率 {{ scope.row.feeRate }}</small></template></ElTableColumn><ElTableColumn label="生效时间" min-width="155"><template #default="scope">{{ scope.row.effectiveDate }}<small class="table-subtext">{{ scope.row.expireDate || "长期有效" }}</small></template></ElTableColumn><ElTableColumn label="状态" width="110"><template #default="scope"><span class="status-badge" :class="'st-' + scope.row.status.toLowerCase()">{{ scope.row.status }}</span></template></ElTableColumn><ElTableColumn label="操作" width="230" align="right"><template #default="scope"><div class="settlement-row-actions"><button class="outline-btn action-btn" type="button" title="查看规则详情" @click="openDetail(scope.row as SettlementRule)"><Eye :size="15" />详情</button><button v-if="hasPermission('settlement:rule:manage')" class="outline-btn action-btn" type="button" title="编辑规则" :disabled="saving" @click="openEdit(scope.row as SettlementRule)"><Pencil :size="15" />编辑</button><button v-if="hasPermission('settlement:rule:manage')" class="outline-btn action-btn" type="button" :title="scope.row.status === 'ACTIVE' ? '停用规则' : '启用规则'" :disabled="saving" @click="toggleRule(scope.row as SettlementRule)"><ToggleRight v-if="scope.row.status === 'ACTIVE'" :size="15" /><ToggleLeft v-else :size="15" />{{ scope.row.status === 'ACTIVE' ? '停用' : '启用' }}</button></div></template></ElTableColumn></ElTable></div>
