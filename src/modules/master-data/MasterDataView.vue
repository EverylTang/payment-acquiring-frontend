...
<template>
  <section class="workspace-panel master-data-view management-list-page">
    <form class="management-filter-form" @submit.prevent="search"><div class="management-filter-fields management-filter-fields-compact"><label class="management-form-item"><span>国家/地区</span><div class="management-input"><Search :size="16" /><input v-model="filters.keyword" placeholder="名称、代码或区域" /></div></label><label class="management-form-item"><span>状态</span><select v-model="filters.status"><option value="">全部状态</option><option value="ACTIVE">已启用</option><option value="DISABLED">已停用</option></select></label></div><div class="management-filter-actions"><button class="outline-btn" type="button" @click="resetFilters">重置</button><button class="primary-btn" type="submit"><Search :size="16" />查询</button><button v-if="hasPermission('master-data:create')" class="primary-btn" type="button" @click="openCountryCreate"><Plus :size="16" />新增国家/地区</button></div></form>
