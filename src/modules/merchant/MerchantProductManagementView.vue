  ...
  <section class="workspace-panel management-list-page merchant-product-management-page">
    <form class="management-filter-form" @submit.prevent="search">
      <div class="management-filter-fields">
        <label class="management-form-item"><span>商户</span><ElSelect v-model="filters.merchantId" clearable filterable :loading="referenceLoading" placeholder="选择商户"><ElOption v-for="merchant in merchants" :key="merchant.merchantId" :label="`${merchant.name} · ${merchant.merchantId}`" :value="merchant.merchantId" /></ElSelect></label>
