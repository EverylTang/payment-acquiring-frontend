...
import { bindMerchantProduct, callbackOrder, cancelOrder, createOrder, createUser, getAdminList, getChannelHealth, getMerchantProducts, getOrder, getOrderHealth, getOrderPage, getOrderStatistics, getOverview, getRoles, getSnapshot, getUsers, type AdminRecord, type AdminRole, type AdminUser, type CreateOrderRequest, type DashboardOverview, type MerchantProduct, type Order, type OrderPage } from './api';
...
const loadUsers = async () => {
  listLoading.value = true;
  try { [users.value, roles.value] = await Promise.all([getUsers(), getRoles()]); } catch (error) { notice.value = error instanceof Error ? error.message : '用户加载失败'; } finally { listLoading.value = false; }
};
