  ...
  <main class="login-page">
    <section class="login-intro">
      ...
      <div>
        ...
        <h1>让每一笔交易，<br />都在掌控之中。</h1>
        <p>统一管理支付配置、渠道路由、风险策略与资金流向。</p>
      </div>
      <div class="security-note">
        <ShieldCheck :size="18" /><span>受 JWT 与角色权限保护</span>
      </div>
    </section>
    <section class="login-panel">
      <form @submit.prevent="submit">
        ...
        <p>请输入管理员账号继续。</p>
        <label
          >账号<input v-model="username" autocomplete="username" required
        /></label>
        <label
          >密码<input
            v-model="password"
        ...
        <button class="primary-btn login-submit" :disabled="busy">
          <LoaderCircle v-if="busy" class="spin" :size="17" /><template v-else
